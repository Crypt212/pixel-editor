...
type ColorParams = (
    | { rgb: ColorVector; hsl?: never; hex?: never }
    | { hsl: ColorVector; rgb?: never; hex?: never }
    | { hex: string; rgb?: never; hsl?: never }
) & {
...
class Color {
     ...
     * @private
     */
    private static cachedColors: Map<string, Color> = new Map();
     ...
     */
    constructor(colorData: ColorData, key: symbol) {
    ...
    get rgb(): ColorVector { return [...this.data.rgb]; }

    /** @returns {ColorVector} HSL values [h, s, l] (h: 0-360, s/l: 0-100) */
    get hsl(): ColorVector { return [...this.data.hsl]; }

    /** @returns {string} Hex color string */
    get hex(): string { return this.data.hex; }

    /** @returns {number} Alpha value (0.0-1.0) */
    get alpha(): number { return this.data.alpha; }

    /** @returns {string} Hex representation */
    toString(): string { return this.data.hex; }
     ...
     */
    static get(params: ColorParams): Color {

        const alpha = params.alpha ?? 1;

        let key: string,
            finalRGB: ColorVector = [0, 0, 0],
            finalHSL: ColorVector = [0, 0, 0],
            finalHEX: string,
            finalAlpha: number;
    ...
    static mix(
        ...
        color2: Color,
        weight: number = 0.5,
        mode: ColorSpace = ColorSpace.rgb
    ): Color {
     ...
     */
    static compositeOver(topColor: Color, bottomColor: Color): Color {
        ...
        const combinedAlpha: number = aTop + aBottom * (1 - aTop);
     ...
     */
    withRGB(rgb: ColorVector = [... this.data.rgb]): Color {
     ...
     * @returns {Color} New color instance
     */
    withHSL(hsl: ColorVector = [... this.data.hsl]): Color {
     ...
     * @static
     */
    static TRANSPARENT: Color = this.get({ rgb: [0, 0, 0], alpha: 0 });
 ...
 * @private
 */
function rgbToHsl(rgb: ColorVector): ColorVector {
    let [r, g, b] = rgb;
    r /= 255;
    g /= 255;
    b /= 255;
 ...
 * @private
 */
function hslToRgb(hsl: ColorVector): ColorVector {
    let [h, s, l] = hsl;
    h = h % 360 / 360;
 ...
 * @private
 */
function toHex(rgb: ColorVector, alpha: number): string {
    const components = [
        Math.round(rgb[0]),
        Math.round(rgb[1]),
        Math.round(rgb[2]),
 ...
 * @private
 */
function validateRGB(rgb: ColorVector) {
 ...
 * @private
 */
function validateHSL(hsl: ColorVector) {
 ...
 * @private
 */
function parseHex(hex: string): { rgb: ColorVector, alpha: number } {
