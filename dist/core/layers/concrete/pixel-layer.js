...
export default class PixelLayer {
    ...
    undo() {
        ...
        if (this.history.atStart)
            return;
        this.history.undo();
        this.applyRecord(1 /* HistoryMove.Backward */);
    }
    /**
     * Redos an action
     * @method
    ...
    redo() {
        ...
        if (this.history.atEnd)
            return;
        this.history.redo();
        this.applyRecord(0 /* HistoryMove.Forward */);
    }
    /** helper method */
    applyRecord(direction) {
        const record = this.history.getRecordData();
        let state;
        if (direction === 0 /* HistoryMove.Forward */)
            state = "after";
        else if (direction === 1 /* HistoryMove.Backward */)
            state = "before";
