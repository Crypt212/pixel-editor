...
class DrawingTool {
    ...
    constructor(layerSystem) {
        if (!(layerSystem instanceof LayerManager))
            throw new TypeError(
    ...
    action(pixelPosition) {
        const pixelOperation = (x, y) => {
            ...
            let oldColor = this.#layerManager.getLayerCanvas().getColor(x, y);
            ...
            if (
                ...
                return;
            this.#layerManager.getLayerHistory().addActionData({
                x: x,
                y: y,
                colorOld: oldColor,
                colorNew: newColor,
        ...
        const history = this.#layerManager.getLayerHistory();
    ...
    endAction() {
        ...
        this.#isActionStart = false;
        if (this.#layerManager.getLayerHistory().getActionData().length === 0)
            this.#layerManager.undo(); // action does nothing, remove it
        this.resetBuffer(this.#currentRect);
    ...
    fill(x, y, color, tolerance = 0, pixelOperation) {
        if (
            x < 0 ||
            y < 0 ||
            x >= this.#layerManager.getWidth ||
            y >= this.#layerManager.getHeight
        )
            return;

        const canvas = this.#layerManager.getLayerCanvas();
